...
class ProhibitedClaimGuardTest {
  ...
  @Test void claimWithoutTermsUsesNarrowCompliancePrefixFallback(){var r=guard.check(null,"  治疗脱发！ ",List.of(claim("f4","v4","{\"claimText\":\"不得宣称治疗脱发\"}")),List.of("EXISTING"));assertTrue(r.hit());assertEquals("治疗脱发",r.matchedClaims().getFirst().get("matchedText"));assertTrue(r.riskFlags().contains("EXISTING"));}
  @Test void nullBlankPunctuationAndUnrelatedContentAreSafe(){assertFalse(guard.check(null,null,List.of(claim("f5","v5","{\"claimText\":\"！！\"}")),List.of()).hit());assertFalse(guard.check("","行业很好",List.of(claim("f6","v6","{\"claimText\":\"不得宣称\"}")),List.of()).hit());assertFalse(guard.check("","普通门店介绍",List.of(claim("f7","v7","{\"claimText\":\"不得宣称治疗脱发\"}")),List.of()).hit());}
}
