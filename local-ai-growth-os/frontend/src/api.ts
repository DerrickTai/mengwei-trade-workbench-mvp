...
function handleUnauthorized(){ logout(); window.dispatchEvent(new Event('local-growth-auth-expired')) }
export async function api(path:string, options:RequestInit={}) { const isForm=options.body instanceof FormData; const headers={ ...(isForm?{}:{'Content-Type':'application/json'}), ...(token()?{Authorization:`Bearer ${token()}`}:{ }), ...(options.headers||{}) }; const res=await fetch(`/api/v1${path}`,{...options,headers}); if(res.status===401) handleUnauthorized(); const body=await res.json().catch(()=>({})); if(!res.ok) throw new Error(body.message||'请求失败'); return body }
...
export async function apiBlob(path:string){const res=await fetch(`/api/v1${path}`,{headers:{...(token()?{Authorization:`Bearer ${token()}`}:{})}});if(res.status===401) handleUnauthorized();if(!res.ok){const type=res.headers.get('content-type')||'';if(type.includes('application/json')){const body=await res.json();throw new Error(body.message||'请求失败')}throw new Error('下载失败')}const filename=(res.headers.get('content-disposition')||'').match(/filename=([^;]+)/)?.[1]?.replace(/"/g,'')||null;return{blob:await res.blob(),filename}}
