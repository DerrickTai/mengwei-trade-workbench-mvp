...
## 13. 已实现补充：M5.2 自动复测与统计归因

M5.2 在 M5.1 证据闭环之后增加复测实验、目标/对照问题组、按问题 × Collector × 重复次数的采样计划、Wilson 置信区间、波动提示、MRR 与简化差分比较。计划使用 `ZoneId` 计算 D+3、D+7、D+14、D+30 等时间点；数据库 lease 与 `SKIP LOCKED` 为多实例领取提供边界。

Worker 默认关闭，自动执行仍只生成 staging 与 DRAFT 观察，正式 M5 口径保持不变。自动归因始终为待人工复核的草案；直接引用不等同于指标变化的确定原因，域名级匹配不算直接引用。
